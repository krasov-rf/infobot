...
type IInfoBotDB interface {
	...
	Feedbacks(ctx context.Context, opt *OptionsInfoBot) ([]*serializers.ConversionSerializer, int, error)
	// Добавить пользовательское обращение
	FeedbackInsert(ctx context.Context, user *serializers.ConversionSerializer) error
	...
	MonitoringSiteAdd(ctx context.Context, user_id int64, site_url string) (*serializers.SiteSerializer, error)
	// обновить сайт
